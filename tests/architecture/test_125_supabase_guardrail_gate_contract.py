...
def test_baseline_report_captures_billing_plan_and_guardrail_status() -> None:
    ...
    assert "Spend Guardrails API Coverage + Dashboard Evidence" in src
    assert "Spend alerts: `50%`, `80%`, `95%`" in src
    assert "monthly hard cap" in src
...
def test_dry_run_report_links_guardrail_blockers_to_baseline() -> None:
    ...
    assert "Spend alerts (50/80/95) + monthly hard cap were configured in dashboard on 2026-02-16." in src
